...
# Step Functions State Machine ARN (NEW - add this to env vars)
...
def trigger_step_functions(load_dt: str, bucket: str, file_count: int):
    ...
    try:
        execution_name = f"pipeline-{load_dt}-{datetime.now(timezone.utc).strftime('%H%M%S')}"
        ...
        response = stepfunctions.start_execution(
            ...
            input=json.dumps({
                "bucket": bucket,
                "prefix": f"{RAW_PREFIX}/google_drive/",
                "date": load_dt,
    ...
    try:
        ...
        try:
            execution_table.update_item(
                ...
                UpdateExpression="SET #status = :status, end_time = :end_time, files_processed = :count, file_list = :files, stepfunctions_execution = :sf_exec",
...
Add this new environment variable to your `healthcare-drive-to-s3-ingestion` Lambda:

**In Lambda Console → Configuration → Environment variables → Edit:**

Add:
```
STATE_MACHINE_ARN = arn:aws:states:us-east-1:YOUR_ACCOUNT_ID:stateMachine:healthcare-data-pipeline-orchestrator
```
